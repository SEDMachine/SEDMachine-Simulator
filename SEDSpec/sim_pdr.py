'''
Created from sim2.py on 6 nov 2011 by npk
'''
...
for phiix in [0,1,2,3,4,5,6]:
	...
	config = {"GAIN": 2, "pix_per_res": 2.4, "extract_pixs": 6, "extract_diameter_spaxels": 3, "camera_name": "PI", 'disperser': 'prism'}
	observation = {"telarea_cm2": 18242. * 0.9, "exptime": 1200, "n_exp": 3, "airmass": 1.2, "PHASE_IX": phiix, "seeing_fwhm_as": 2.2, "spaxel_pitch_as": 0.65, "source_AB": 20.5}
...
fname = now.strftime("pdr-phase-%Y-%m-%d__%H-%M-%S") + ".pdf"
pl.figure(0)
pl.clf()
pos = .9
...
pl.figure(6)
pl.clf()
pl.title("DQE PI eXcelon v Andor")
...
for fwhm in fwhms:
    ...
    for spax in spax_size:
        ...
        observation["spaxel_pitch_as"] = spax

        ext_diam = fwhm * 2 / spax
        #config["extract_diameter_spaxels"] = np.ceil(ext_diam)

        ls, nsky_photon, nsource_photon, shot_noise, dark, RN, fraction, qe, atm = calculate(config, observation, 13)

        pl.figure(8)
        pl.plot(ls, nsource_photon/shot_noise, lw=lw)
        legend.append("fwhm: %s\" spax: %s\" f: %0.2f" % (fwhm, spax, fraction))
        print "FWHM: %s  Spax: %s   RN: %i" % (fwhm, spax, RN)
		
pl.figure(8)
pl.legend(legend)
pl.title(u"SNR per seeing and spaxel. Extract %i spaxels (%i $e^-$ RN per R)" % (config["extract_diameter_spaxels"], RN))
pl.ylabel("SNR per Resolution Element")
pl.xlabel(u"$\lambda$ [$\AA$]")
observation["seeing_fwhm_as"] = 2.2
observation["spaxel_pitch_as"] = 0.675
