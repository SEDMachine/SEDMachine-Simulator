...
ext = sp.interpolate.interp1d(palextinct[:, 0], palextinct[:,1], kind='cubic')

pl.figure(2)
pl.clf()
lam = palextinct[:,0]
pl.plot(lam, 10**(-ext(lam)/2.5))
...
def source(lam):
	...
	return ff(lam)#*10**(-ext(lam)*airmass/2.5)

pl.figure(1)
l = np.arange(3300, 10500,200)
pl.plot(l, source(l)*hc/l)
pl.xlabel(u"$\lambda$ [$\AA$]")
pl.ylabel(u"flux [$erg$ $s^{-1}$ $cm^{-2}$ $\AA^{-1}$]")
pl.xlim([3700,10000])
pl.legend(["Sky", "Quimby sky", "21 mag source"], loc="upper left")
pl.title("Source and Sky Spectrum")
pl.show()
pl.savefig("flux.pdf")


def instrument_efficiency(l):
	return 0.05 * qe(l)

def num_photons(fun, exptime, area, lam1, lam2):
	num_photon = sp.integrate.quad(fun, lam1, lam2, limit=250)[0] * exptime * area 
...
while startl < 10500:
	l = (startl+endl)/2
	nsky_photon.append(num_photons(skyf, exptime, kpnoarea, startl, endl) * 
		instrument_efficiency(l))
	nsource_photon.append(num_photons(source, exptime, kpnoarea, startl, endl) * 
		instrument_efficiency(l))
	ls.append(l)
...
print "Area: %6.0f cm^2 Exptime: %6.0f s" % (kpnoarea, exptime)
print
loi = 7000

i6000 = np.argmin(np.abs(ls-loi))
print "At %i nm:" % (loi/10)
print "Efficiency: %1.2f" % (instrument_efficiency(loi))
sky6000 = num_photons(skyf, exptime, kpnoarea, loi-loi/R/2., loi+loi/R/2.) * \
	instrument_efficiency(loi)
source6000 = num_photons(source, exptime, kpnoarea, loi-loi/R/2., loi+loi/R/2.) * \
	instrument_efficiency(loi)
...
print "  source noise: %6.1f sky noise: %6.1f  RN: %6.1f DCN: %6.1f per R" % (
	...
	np.sqrt(npix*RN**2), np.sqrt(npix*DC*exptime))
...
print "Source #/s/A/cm^2: %6.1e sky: %6.1e (modulo efficiency)" % (
	source6000 / exptime / kpnoarea / dlambda,
	sky6000 / exptime / kpnoarea / dlambda)

pl.figure(3)
pl.clf()
pl.title("Detected Counts")
pl.semilogy(ls, nsource_photon, linewidth=2)
pl.semilogy(ls, np.sqrt(nsky_photon + nsource_photon + npix*RN**2 + npix*DC*exptime),linewidth=3)
...
pl.ylim([10,1e5])
pl.xlabel(u"$\lambda$ [$\AA$]")
pl.ylabel("Counts per Resolution Element (%i s exp)" % exptime)

pl.legend(["21 mag Source", "Noise", "Sky", "RN=%i/pix" % (RN/np.sqrt(3)), "DC=%1.3f/pix/s" % (DC)], loc='upper left')

ax2 = pl.twinx()
l = np.arange(3500,11000)
ax2.plot(l,instrument_efficiency(l), '--')
pl.ylim([0,1])
pl.ylabel("Instrument QE")

pl.xlim([3500,10000])
pl.savefig("counts.pdf")

[nsky_photon, nsource_photon, ls] = map(np.array, [nsky_photon, nsource_photon, ls])

shot_noise = np.sqrt(nsky_photon + nsource_photon + npix * RN**2 + npix * DC * exptime)

fig = pl.figure(4)
pl.clf()
pl.axvline(3700)
ax1 = fig.add_subplot(111)
pl.title("S/N for 21 mag source")
